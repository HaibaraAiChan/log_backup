...
def run(args, device, data, tic):
	...
	for epoch in range(args.num_epochs):
		...
		for step, (input_nodes, seeds, blocks) in enumerate(training_dataloader):
			...
			for index, param in enumerate(model_engine.parameters()):
		...
		for index, param in enumerate(gradients):
			# param.grad = torch.tensor(gradients[index])
			param.grad = gradients[index].clone().detach()
		gradients = []
